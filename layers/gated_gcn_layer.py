...
class GatedGCNLayer(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, dropout, graph_norm, batch_norm, layer_norm, gated_gcn_agg):#, residual=False):
        ...
        self.C = nn.Linear(input_dim, output_dim, bias=False)
        self.D = nn.Linear(input_dim, output_dim, bias=False)
        self.E = nn.Linear(input_dim, output_dim, bias=False)
        if batch_norm:
    ...
    def message_func(self, edges):
        ...
        e_ij = F.relu(e_ij) #   be noticed, older models does not have this relu
        edges.data['e'] = e_ij
