...
'''
ref: https://github.com/snap-stanford/pretrain-gnns (Strategies for Pre-training Graph Neural Networks)
'''
...
def random_scaffold_split(dataset, smiles_list, task_idx=None, null_value=0,
    ...
    for ind, smiles in smiles_list:
        #   "No molecule provided" error occurs. temporally ignoring such results
        try:
            scaffold = generate_scaffold(smiles, include_chirality=True)
            scaffolds[scaffold].append(ind)
        except:
            print('No molecule provided error occurred!')
    ...
    train_smiles = [smiles_list[x][1] for x in train_idx]
    valid_smiles = [smiles_list[x][1] for x in valid_idx]
    test_smiles = [smiles_list[x][1] for x in test_idx]
